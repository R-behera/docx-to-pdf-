...
app.post('/convert', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const exportTask = status.data.tasks.find(t => t.operation === 'export/url' && t.result?.files?.length > 0);
      if (exportTask) {
    ...
    if (!pdfUrl) return res.status(500).json({ error: 'Timeout waiting for PDF' });
